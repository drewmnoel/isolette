...
\reqm{ENV}
{The current temperature received from the sensor is a a real number in the range 68.0$\degree{F}$ to 105.0$\degree{F}$.\\}
{\hl{TODO}}
\label{E5}
\textbf{Rationale}: The sensor has some given range, which allows for restriction of the expected input.
